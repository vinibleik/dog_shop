...
describe("Product", () => {
    ...
    const price = 9.99;
    const full_product = new Product.Product(name, price, img);
    const img_missing = new Product.Product(name, price);
    ...
    describe("#seachId", () => {
        test("Should return the right products", () => {
            expect(Product.Product.searchId(0)).toEqual(full_product);
            expect(Product.Product.searchId(1)).toEqual(img_missing);
            expect(Product.Product.searchId(1.23)).toEqual(img_missing);
        });

        test("Should return null for invalid ids", () => {
            expect(Product.Product.searchId(-1)).toEqual(null);
            expect(Product.Product.searchId(2)).toEqual(null);
        });

        test("Should throw an error", () => {
            expect(() => {
                Product.Product.searchId("test");
            }).toThrow(TypeError);
        });
    });
});
